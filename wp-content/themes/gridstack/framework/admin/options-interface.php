...
function optionsframework_fields() {
	...
	//$themename = wp_get_theme(STYLESHEETPATH); 
	$themename = 'GridStack';
	...
	foreach ($options as $value) {
		...
		case "checkbox":
			...
			$output .= '<label for="' . esc_attr( $value['id'] ) . '">' . wp_kses( $explain_value, $allowedtags) . '</label>';
		break;
		...
		case 'typography':	
			...
			// Font Weight 2
			$output .= '<select class="of-typography of-typography-style" name="'.$option_name.'['.$value['id'].'][style2]" id="'. $value['id'].'_style2">';

			/* Font Style 2*/
			$styles2 = of_recognized_font_styles2();
			foreach ( $styles2 as $key => $style2 ) {
				$output .= '<option value="' . esc_attr( $key ) . '" ' . selected( $typography_stored['style2'], $key, false ) . '>'. $style2 .'</option>';
			}
			$output .= '</select>';
		...
		// Typography
		case 'typography_nosize':	
		
			$typography_nosize_stored = $val;
			...
			$faces = of_recognized_font_faces();
			foreach ( $faces as $key => $face ) {
				$output .= '<option value="' . esc_attr( $key ) . '" ' . selected( $typography_nosize_stored['face'], $key, false ) . '>' . esc_html( $face ) . '</option>';
			}			
			...
			$styles = of_recognized_font_styles();
			foreach ( $styles as $key => $style ) {
				$output .= '<option value="' . esc_attr( $key ) . '" ' . selected( $typography_nosize_stored['style'], $key, false ) . '>'. $style .'</option>';
			}
			$output .= '</select>';
			
			// Font Weight
			$output .= '<select class="of-typography of-typography-style" name="'.$option_name.'['.$value['id'].'][weight]" id="'. $value['id'].'_weight">';

			/* Font Style */
			$weights = of_recognized_font_weight();
			
			if (!isset($typography_nosize_stored['weight'])) $typography_nosize_stored['weight'] = '400';
			foreach ( $weights as $key => $weight ) {
				$output .= '<option value="' . esc_attr( $key ) . '" ' . selected( $typography_nosize_stored['weight'], $key, false ) . '>'. $weight .'</option>';
			}
			$output .= '</select>';
